...
if __name__ == "__main__":
    ...
    # public_firms = select_observations(host, database_public_firms, user, password, schema_public_firms, table_public_firms, "*")
    # public_firms.to_csv("public_firms.csv", index=False, sep="|")
    public_firms = pd.read_csv("public_firms.csv", sep="|")
    
    # obtaining data from table 1
    # table_1 = select_observations(host, database_table_1, user, password, schema_table_1, table_table_1, "*")
    # table_1.to_csv("table_1.csv", index=False, sep="|")
    table_1 = pd.read_csv("table_1.csv", sep="|")
    #table_1["issue_date"] = table_1["issue_date"].apply(lambda issue_date: clean_date_val(issue_date))
    ...
    print("reports_entities_names: ", len(reports_entities_names))
    ...
    # matches_df_table_1["matches"] = matches_df_table_1["entity"].apply(lambda name: find_match(name, dict_closest_matches_reports))
    # matches_df_table_1.to_csv("matches_df_table_1.csv", index=False, sep="|")
    # matches_df_table_1["matches"].value_counts().to_csv("matches_public_firms_count_alt.csv", sep="|")

    ## reading fuzzy matched data
    # matches_df_public_firms = pd.read_csv("matches_df_public_firms.csv", sep="|")
    # matches_df_public_firms["matches"].value_counts().to_csv("matches_public_firms_count.csv", sep="|")
